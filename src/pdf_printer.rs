...
pub fn compile(parsed_data: Vec<(Rule, Vec<FragmentType>)>) {
    ...
    for data in parsed_data {
        current_layer = convert_header(current_layer, y_val, &font, data);
        y_val -= 20.0;
    }
...
fn convert_header(current_layer: PdfLayerReference, y_val: f32, font: &IndirectFontRef, data: (Rule, Vec<FragmentType>)) -> PdfLayerReference {
    
    let content = merge_sentence(data.1);
    let size = set_header_size(data.0);
...
fn set_header_size(icon : Rule) -> f32 {
    let mut size = 0.0;
    match icon {
        Rule::H1 => {size = 30.0},
        Rule::H2 => {size = 25.0},
